...
-- WITH rnd1_prelim AS (        
    SELECT
        ...
        ROUND(overall_pick_number - projection_adp, 2) AS adp_vs_projected_pick,                 -- positive means drafted later than projected        
        --- !!make cumul version of above.  Hypo: Matters more in early rounds than later
        SUM(adp_vs_projected_pick) OVER(PARTITION BY team_id 
                                        ORDER BY team_pick_number ASC) AS adp_vs_projected_cumul,
        ...
        COUNT(CASE WHEN position_name = 'RB' THEN player_name END) OVER(
                ...
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)           AS rb_cnt_cumul,
        COUNT(CASE WHEN position_name = 'WR' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id ORDER BY team_pick_number
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)           AS wr_cnt_cumul,
        COUNT(CASE WHEN position_name = 'QB' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id ORDER BY team_pick_number
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)           AS qb_cnt_cumul,
        COUNT(CASE WHEN position_name = 'TE' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id ORDER BY team_pick_number
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)           AS te_cnt_cumul,                                                                                 
        COUNT(CASE WHEN position = 'RB' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id)                           AS rb_cnt_total,
        COUNT(CASE WHEN position = 'WR' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id)                           AS wr_cnt_total,
        COUNT(CASE WHEN position = 'QB' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id)                           AS qb_cnt_total,
        COUNT(CASE WHEN position = 'TE' THEN player_name END) OVER(
                PARTITION BY tournament_entry_id)                           AS te_cnt_total                                            
        --     -- points per position
...
SELECT 
    r1.*,
    SUM(r1.adp_vs_projected) OVER(PARTITION BY r1.team_id 
                                  ROWS BETWEEN UNBOUNDED PRECEDING AND 
                                  CURRENT ROW) AS adp_vs_projected_total,  -- Team Total: adp vs projected (positive means drafted later than projected)    
